...
class DBWorker():
    ...
    def db_chk_user(self, login):
        ...
        if (pattern.fullmatch(login) is None) or (len(login) < 3):
            ...
            exit(1)
        if (self.curs.execute(f'SELECT "login" FROM "users" WHERE "users"."login" = "{login}";').fetchall()):
            return(True)
        return(False)
    ...
    def db_userinfo(self, args):
        ...
        '''
        if self.db_chk_user(args.login):
            if self.curs.execute(f'SELECT "active" FROM "users" WHERE "users"."login" = "{args.login}";').fetchall():
    ...
    def auth(self, args):
        ...
        # login, passord = authfile.readline().split('@')
        login, password = [i.strip() for i in authfile.readlines()]
        authfile.close()

        if self.db_chk_user(login):
            password2 = self.curs.execute(f'SELECT "password" FROM "users" WHERE "users"."login" = "{login}";').fetchall()
